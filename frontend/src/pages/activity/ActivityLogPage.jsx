...
function ActivityLogPage() {
    ...
    const fetchActivityData = async () => {
      try {
        ...
        const response = await activityService.getUsersActivity({
          limit_per_user: 10, // Mostra le 10 attività più recenti per ogni utente
  ...
  const activeUsers = [...usersActivity].sort((a, b) => b.activities_count - a.activities_count);
          ...
          <Grid container spacing={2}>
            {activeUsers.length > 0 ? (
              activeUsers.map((userData) => (
                <Grid item xs={12} md={6} key={userData.professor_id}>
                  <ActivitySummaryCard activityData={userData} />
                </Grid>
              ))
            ) : (
              <Grid item xs={12}>
